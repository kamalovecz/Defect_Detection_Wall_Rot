...
from training_project.ablations.run_neu_det_300ep import RESULT_COLUMNS, validate_results_csv
from training_project.config import load_config
from training_project.train import dataset_name
from ultralytics.nn.tasks import DetectionModel

CONTRACT_PATH = ROOT / "training_project/ablations/neu_det_dataset_contract.json"
...
def validate_dataset(contract: dict) -> dict:
    ...
    for split in ("train", "val", "test"):
        ...
        images_root = split_root / "images" if (split_root / "images").is_dir() else split_root
        labels_root = split_root / "labels"
        images = sorted(
            (path for path in images_root.rglob("*") if path.suffix.lower() in IMAGE_EXTENSIONS),
            key=lambda path: path.as_posix(),
        )
        labels = sorted(labels_root.rglob("*.txt"), key=lambda path: path.as_posix())
        ...
        for image in images:
            ...
            if not label.is_file():
                raise RuntimeError(f"Missing NEU-DET label: {split}/{relative.as_posix()}")
            ...
            for line_number, row in enumerate(label.read_text(encoding="utf-8").splitlines(), 1):
                parts = row.split()
                if len(parts) != 5:
                    raise RuntimeError(f"Invalid NEU-DET row width: {label}:{line_number}")
                cls = int(parts[0])
                coords = [float(value) for value in parts[1:]]
                if not 0 <= cls < contract["nc"] or not all(0.0 <= value <= 1.0 for value in coords):
                    raise RuntimeError(f"Invalid NEU-DET row range: {label}:{line_number}")
                instances += 1
                classes[cls] += 1
            for path in (image, label):
                rel = path.relative_to(split_root).as_posix()
        ...
        if summary != contract["splits"][split]:
            raise RuntimeError(f"NEU-DET {split} contract changed: {summary}")
    ...
    if combined.hexdigest() != contract["combined_content_fingerprint_sha256"]:
        raise RuntimeError("NEU-DET combined content fingerprint changed")
    if duplicate_count != contract["cross_split_duplicate_images"] or duplicate_count != 0:
        raise RuntimeError(f"NEU-DET cross-split duplicates changed: {duplicate_count}")
    return observed
...
def main() -> int:
    ...
    if dataset_name(contract["descriptor"]) != contract["dataset"]:
        raise RuntimeError("NEU-DET manifest dataset name derivation changed")
    ...
    for experiment_id, config_relative, model_relative in EXPERIMENTS:
        ...
        if list(prediction.shape) != [1, 10, 8400]:
            raise RuntimeError(f"NEU-DET output shape mismatch for {experiment_id}: {list(prediction.shape)}")
