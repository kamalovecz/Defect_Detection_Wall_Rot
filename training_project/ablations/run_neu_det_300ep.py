"""Run the six canonical ablation models sequentially on NEU-DET and validate each best checkpoint."""
...
def validate_dataset_contract() -> None:
    subprocess.run(
        [sys.executable, "training_project/ablations/verify_neu_det_ablation.py", "--dataset-only"],
        cwd=ROOT,
        check=True,
        capture_output=True,
    )
...
def main() -> int:
    ...
    parser.add_argument("--project", default="training_project/runs/neu_det_ablation_300ep_b8_20260721")
    parser.add_argument("--validation-project", default="training_project/runs/neu_det_ablation_300ep_b8_20260721_val")
    ...
    contract = json.loads((ROOT / "training_project/ablations/neu_det_dataset_contract.json").read_text(encoding="utf-8"))
    ...
    if project.exists() or validation_project.exists():
        raise FileExistsError("NEU-DET output project already exists; refusing to overwrite")

    assert_git_identity(subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip())
    commit = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()
    subprocess.run([sys.executable, "training_project/ablations/verify_neu_det_ablation.py"], cwd=ROOT, check=True)
