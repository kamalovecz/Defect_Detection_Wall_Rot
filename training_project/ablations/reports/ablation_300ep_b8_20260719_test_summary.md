# HARP-Net 六模型 Test 数据集检测报告

## 1. 报告用途

本报告记录六个已锁定 `best.pt` 在 Port_Defect **test split** 上的一次性评估结果。模型训练与 checkpoint 选择已经在 train/val 流程中完成，本次 test 评估不参与调参、早停、checkpoint 选择或模型重训。
...
在各模型评估开始前，对应 test 输出目录均不存在；六个 checkpoint、外部验证脚本和数据合同均通过哈希与数量检查。六组 test 评估均以退出码 0 完成。测试 manifest 使用训练清单中锁定的 checkpoint SHA-256 作为测试前期望值，并记录测试后的重新计算值，六组均一致。
...
除 B4 外的五个模型，其 test mAP50-95 均低于对应 val 结果；B4 则高出 0.0106。跨 split 差值只能描述当前两个划分的数值变化，不能替代跨 seed 稳定性验证。
...
- **观察**：B5 的 F1 为 0.7631，为六组最高；mAP50-95 为 0.5000，比 A0 高 0.0273。
