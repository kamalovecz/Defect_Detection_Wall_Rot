...
def build_yolo_cls_loss():
    """Build classification loss for YOLO detection heads from runtime env settings."""
    loss_type = str(os.getenv("YOLO_CLS_LOSS", "bce")).strip().lower()
    if loss_type == "focal":
        alpha = _get_env_float("YOLO_FOCAL_ALPHA", 0.25)
        gamma = _get_env_float("YOLO_FOCAL_GAMMA", 1.5)
        print(f"[Loss] classification loss: focal (alpha={alpha}, gamma={gamma})")
        return FocalLoss_YOLO(alpha=alpha, gamma=gamma), loss_type
    if loss_type == "varifocal":
        alpha = _get_env_float("YOLO_VARIFOCAL_ALPHA", 0.75)
        gamma = _get_env_float("YOLO_VARIFOCAL_GAMMA", 2.0)
        print(f"[Loss] classification loss: varifocal (alpha={alpha}, gamma={gamma})")
        return VarifocalLoss_YOLO(alpha=alpha, gamma=gamma), loss_type
    if loss_type != "bce":
        print(f"[Loss][WARN] unsupported YOLO_CLS_LOSS={loss_type}, fallback to BCEWithLogitsLoss.")
    print("[Loss] classification loss: bce")
    return nn.BCEWithLogitsLoss(reduction="none"), "bce"
