...
def resolve_model_module(name, layer_index):
    """Resolve a YAML module token without importing legacy extra_modules."""
    ...
    try:
        ...
        if name.startswith("torchvision.ops."):
            return getattr(torchvision.ops, name[len("torchvision.ops."):])
        if name.startswith("ops."):
            return getattr(torchvision.ops, name[4:])
            ...
            if m == 'node_mode':
                ...
                if len(args) > 0:
                    if args[0] == 'head_channel':
                        args[0] = int(d[args[0]])
            t = m
            m = resolve_model_module(m, i)  # get module
        except Exception:
            if isinstance(m, str):
                raise
            pass
