...
class ADown(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        self.cv2 = Conv(c1 // 2, self.c, 1, 1, 0) # 压缩通道和融合

    def forward(self, x):
        """Forward pass through ADown layer."""
        x = torch.nn.functional.avg_pool2d(x, 2, 1, 0, False, True) # 预处理
        x1, x2 = x.chunk(2, 1)
        x1 = self.cv1(x1)
        x2 = torch.nn.functional.max_pool2d(x2, 3, 2, 1) # 下采样做下采样，更关心显著区域
        x2 = self.cv2(x2)
