...
# ===== AMP decorator compatibility (works across different torch versions) =====
# Provide unified decorators: AMP_CUSTOM_FWD / AMP_CUSTOM_BWD
# Supports both:
#   @AMP_CUSTOM_FWD           or @AMP_CUSTOM_FWD(...)
#   @AMP_CUSTOM_BWD           or @AMP_CUSTOM_BWD(...)

# Prefer torch.amp (newer), fallback to torch.cuda.amp (older)
try:
...
def _make_custom_fwd(device_type="cuda", **kwargs):
    """
    Returns a decorator for forward.
    Compatible with:
      - factory style: custom_fwd(device_type=..., **kwargs)  (newer torch.amp)
      - factory style: custom_fwd(**kwargs)                    (older torch.cuda.amp)
      - decorator style: custom_fwd(fn)
    """
    # Try: torch.amp factory (has device_type)
    try:
        return _cfwd(device_type=device_type, **kwargs)
    except TypeError:
        pass

    # Try: torch.cuda.amp factory (no device_type)
    try:
        return _cfwd(**kwargs)
    except TypeError:
        pass
...
def _make_custom_bwd(device_type="cuda", **kwargs):
    """
    Returns a decorator for backward.
    Compatible with:
      - factory style: custom_bwd(device_type=..., **kwargs)  (some torch.amp)
      - factory style: custom_bwd(**kwargs)                  (rare)
      - decorator style: custom_bwd(fn)                      (common in torch.cuda.amp)
    """
    # Try: torch.amp factory (has device_type)
    try:
        return _cbwd(device_type=device_type, **kwargs)
    except TypeError:
        pass

    # Try: factory without device_type
    try:
        return _cbwd(**kwargs)
    except TypeError:
        pass

    # Fallback: decorator style (most compatible)
    def dec(fn):
        return _cbwd(fn)
    return dec


def AMP_CUSTOM_FWD(*f, **kwargs):
    dec = _make_custom_fwd(**kwargs)
    if f and callable(f[0]):   # used as @AMP_CUSTOM_FWD
        return dec(f[0])
    return dec                 # used as @AMP_CUSTOM_FWD(...)


def AMP_CUSTOM_BWD(*f, **kwargs):
    dec = _make_custom_bwd(**kwargs)
    if f and callable(f[0]):   # used as @AMP_CUSTOM_BWD
        return dec(f[0])
    return dec                 # used as @AMP_CUSTOM_BWD(...)
...
class DCNv3Function(Function):
    @staticmethod
    @AMP_CUSTOM_FWD()  # 也可写成 @AMP_CUSTOM_FWD(cast_inputs=torch.float16)
    def forward(
        ...
        if DCNv3 is None:
            raise ImportError(
                "DCNv3 CUDA extension is not available. "
                "Please install DCNv3 (pip install DCNv3) or disable modules that depend on it."
            )
    ...
    def backward(ctx, grad_output):
        if DCNv3 is None:
            raise ImportError(
                "DCNv3 CUDA extension is not available, cannot run custom backward. "
                "Please install DCNv3."
            )
...
def dcnv3_core_pytorch(
    ...
    spatial_norm = torch.tensor([W_in, H_in]).reshape(1, 1, 1, 2).repeat(
        1, 1, 1, group * (kernel_h * kernel_w - remove_center)
    ).to(input.device)
    ...
    input_ = input.view(N_, H_in * W_in, group * group_channels).transpose(1, 2).reshape(
        N_ * group, group_channels, H_in, W_in)

    # N_, H_out, W_out, group*P_*2 -> N_, H_out*W_out, group, P_, 2 -> N_, group, H_out*W_out, P_, 2
    ...
    # (N_, H_out, W_out, group*P_) -> N_, H_out*W_out, group, P_ -> (N_, group, H_out*W_out, P_)
