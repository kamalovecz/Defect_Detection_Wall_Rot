...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'SEAM', 'MultiSEAM', 'C2f_SWC', 'C3_SWC', 'C3_iRMB', 'C2f_iRMB', 'C3_iRMB_Cascaded', 'C2f_iRMB_Cascaded', 'C3_iRMB_DRB', 'C2f_iRMB_DRB', 'C2f_iRMB_DRBv2','C3_iRMB_SWC', 'C2f_iRMB_SWC','C2f_iRMB_SWCv2',
           ...
           'C3_DynamicConv','C3_DynamicConv_Top1', 'C2f_DynamicConv','C2f_DynamicConv_Top1', 'C3_GhostDynamicConv','C3_GhostDynamicConv_Top1', 'C2f_GhostDynamicConv','C2f_GhostDynamicConv_Top1', 'C2f_GhostDynamicConv_Top1_Align','Dynamic_HGBlock', 'C3_RVB', 'C2f_RVB', 'C3_RVB_SE', 'C2f_RVB_SE', 'C3_RVB_EMA', 'C2f_RVB_EMA',
           ...
           'EUCB_SC', 'EUCB_Lite', 'CSP_MSCB_SC', 'C2f_LEGM', 'C2f_LSBlock', 'C2f_MambaOut_LSConv', 'LoGStem', 'C2f_LFEM', 'C2f_RCB', 'C2f_TransMamba', 'C2f_EVS', 'C2f_EBlock', 'C2f_DBlock'
           ]
...
# class C2f_iRMB(C2f):
#     """Enhanced C2f with iRMB blocks for Metal Surface Defect Detection
    
#     閽堝閲戝睘琛ㄩ潰缂洪櫡妫€娴嬬殑鍒涙柊璁捐锛?#     1. 澶氬昂搴︾己闄锋劅鐭ワ細缁撳悎涓嶅悓iRMB鍙樹綋鎹曡幏涓嶅悓灏哄害鐨勭己闄风壒寰?#     2. 缁嗚妭澧炲己鏈哄埗锛氶拡瀵瑰井灏忕己闄风殑鐗瑰緛澧炲己
#     3. 绾圭悊-缂洪櫡鍒嗙锛氶€氳繃涓嶅悓璺緞鍒嗗埆澶勭悊鑳屾櫙绾圭悊鍜岀己闄风壒寰?#     4. 鑷€傚簲鐗瑰緛铻嶅悎锛氭牴鎹己闄风被鍨嬪姩鎬佽皟鏁寸壒寰佹潈閲?#     """
...
#         # 閽堝閲戝睘琛ㄩ潰缂洪櫡妫€娴嬬殑妯″潡缁勫悎绛栫暐
#         self.m = nn.ModuleList()
        
#         for i in range(n):
#             if n == 1:
#                 # 鍗曞眰鏃朵娇鐢ㄦ爣鍑唅RMB
#                 self.m.append(iRMB(self.c, self.c, window_size=7, se_ratio=0.25))
#             elif n == 2:
#                 # 涓ゅ眰閰嶇疆锛氱骇鑱斿鐞嗙矖缁嗙壒寰?#                 if i == 0:
#                     # 绗竴灞傦細浣跨敤iRMB_SWC鎹曡幏澶ц寖鍥寸汗鐞嗙壒寰?#                     self.m.append(iRMB_SWC(self.c, self.c, dw_ks=7, se_ratio=0.25))
#                 else:
#                     # 绗簩灞傦細浣跨敤鏍囧噯iRMB鍏虫敞灞€閮ㄧ己闄?#                     self.m.append(iRMB(self.c, self.c, window_size=5, se_ratio=0.25))
#             elif n == 3:
#                 # 涓夊眰閰嶇疆锛氭笎杩涘紡鐗瑰緛鎻愬彇
#                 if i == 0:
#                     # 绗竴灞傦細浣跨敤iRMB_DRB鎹曡幏澶氬昂搴︾己闄风壒寰侊紙鍒掔棔銆佽绾圭瓑绾挎€х己闄凤級
#                     self.m.append(iRMB_DRB(self.c, self.c, window_size=7, se_ratio=0.125))
#                 elif i == 1:
#                     # 绗簩灞傦細浣跨敤iRMB_Cascaded澶勭悊涓瓑灏哄害缂洪櫡锛堟枒鐐广€佸嚬鍧戠瓑锛?#                     self.m.append(iRMB_Cascaded(self.c, self.c, num_head=self.c//16, se_ratio=0.25))
#                 else:
#                     # 绗笁灞傦細浣跨敤iRMB_SWC鎹曡幏缁嗗井绾圭悊鍙樺寲锛堝井灏忓垝鐥曘€佺（鎹熺瓑锛?#                     self.m.append(iRMB_SWC(self.c, self.c, dw_ks=5, se_ratio=0.25))
#             else:
#                 # 澶氬眰閰嶇疆锛氭贩鍚堢瓥鐣?#                 if i % 4 == 0:
#                     # 浣跨敤DRB鍙樹綋澶勭悊鎵╁紶鐗瑰緛锛堥€傚悎妫€娴嬩笉瑙勫垯褰㈢姸缂洪櫡锛?#                     self.m.append(iRMB_DRB(
#                         self.c, self.c, 
#                         window_size=7 if i < n//2 else 5,
#                         se_ratio=0.125,
#                         attn_s=True
#                     ))
#                 elif i % 4 == 1:
#                     # 浣跨敤绾ц仈鍙樹綋澧炲己鐗瑰緛浼犻€?#                     self.m.append(iRMB_Cascaded(
#                         self.c, self.c,
#                         num_head=self.c//16,
#                         se_ratio=0.25,
#                         v_group=True if i > n//2 else False
#                     ))
#                 elif i % 4 == 2:
#                     # 浣跨敤SWC鍙樹綋鎹曡幏澶ф劅鍙楅噹鐗瑰緛
#                     self.m.append(iRMB_SWC(
#                         self.c, self.c,
#                         dw_ks=7 if i < n//2 else 5,
#                         se_ratio=0.25
#                     ))
#                 else:
#                     # 浣跨敤鏍囧噯iRMB淇濇寔缁嗚妭
...
#         # 缂洪櫡鐗瑰緛澧炲己妯″潡锛堥拡瀵归噾灞炶〃闈㈢己闄风殑鐗圭偣锛?#         self.defect_enhance = DefectFeatureEnhancer(
#             (2 + n) * self.c, 
#             reduction=16,
#             metal_surface=True
#         )
        
#         # 杈圭紭妫€娴嬭緟鍔╁垎鏀紙甯姪妫€娴嬭绾广€佸垝鐥曠瓑杈圭紭鏄庢樉鐨勭己闄凤級
#         self.edge_branch = EdgeDetectionBranch(self.c) if n >= 2 else None
        
#         # 绾圭悊-缂洪櫡鍒嗙娉ㄦ剰鍔?#         self.texture_defect_attn = TextureDefectSeparation((2 + n) * self.c)

#     def forward(self, x):
#         """
#         鍓嶅悜浼犳挱锛岄拡瀵归噾灞炶〃闈㈢己闄锋娴嬩紭鍖?#         """
#         # 鍒濆鐗瑰緛鍒嗗壊
#         y = list(self.cv1(x).chunk(2, 1))
        
#         # 杈圭紭鐗瑰緛鎻愬彇锛堝鏋滃惎鐢級
#         edge_feat = None
#         if self.edge_branch is not None and len(y) > 0:
#             edge_feat = self.edge_branch(y[-1])
        
#         # 閫愬眰澶勭悊锛屽甫鐗瑰緛澧炲己
#         for i, m in enumerate(self.m):
#             if i == 0:
#                 # 绗竴灞傜洿鎺ュ鐞?#                 feat = m(y[-1])
#             else:
#                 # 鍚庣画灞傚姞鍏ヨ法灞傝繛鎺ュ拰杈圭紭鐗瑰緛
#                 if edge_feat is not None and i == len(self.m) // 2:
#                     # 涓棿灞傝瀺鍏ヨ竟缂樼壒寰?#                     feat = m(y[-1] + 0.1 * edge_feat)
#                 else:
#                     # 杞诲井鐨勭壒寰侀噸鐢紝淇濇寔缁嗚妭
#                     feat = m(y[-1] + 0.05 * y[1])
...
#         # 绾圭悊-缂洪櫡鍒嗙澶勭悊
#         concat_feat = self.texture_defect_attn(concat_feat)
        
#         # 缂洪櫡鐗瑰緛澧炲己
#         concat_feat = self.defect_enhance(concat_feat)
        
#         # 鏈€缁堣瀺鍚?#         out = self.cv2(concat_feat)
...
#     涓撻棬閽堝閲戝睘琛ㄩ潰缂洪櫡鐨勭壒寰佸寮猴紝鍖呮嫭锛?#     - 瀵规瘮搴﹀寮猴細鎻愬崌缂洪櫡涓庤儗鏅殑宸紓
#     - 澶氬昂搴︾壒寰佽仛鍚堬細鎹曡幏涓嶅悓澶у皬鐨勭己闄?#     - 灞€閮?鍏ㄥ眬鐗瑰緛骞宠　锛氬悓鏃跺叧娉ㄧ粏鑺傚拰鏁翠綋
#     """
    
#     def __init__(self, channels, reduction=16, metal_surface=True):
#         super().__init__()
#         self.metal_surface = metal_surface
        
#         # 閫氶亾娉ㄦ剰鍔涳紙澧炲己缂洪櫡鐩稿叧閫氶亾锛?#         self.channel_attn = nn.Sequential(
...
#         # 绌洪棿娉ㄦ剰鍔涳紙瀹氫綅缂洪櫡鍖哄煙锛?#         self.spatial_attn = nn.Sequential(
#             nn.Conv2d(channels, channels // reduction, 1),
...
#     """杈圭紭妫€娴嬭緟鍔╁垎鏀?    
#     涓撻棬妫€娴嬮噾灞炶〃闈㈢殑绾挎€х己闄凤紙瑁傜汗銆佸垝鐥曠瓑锛?#     """
    
#     def __init__(self, channels):
#         super().__init__()

#         self.edge_conv = nn.Conv2d(channels, channels, 3, 1, 1, groups=channels, bias=False)
...
#         edges = torch.abs(edges)  # 鍙栫粷瀵瑰€?
#         edges = self.refine(edges)
        
#         return edges


# class TextureDefectSeparation(nn.Module):
#     """绾圭悊-缂洪櫡鍒嗙娉ㄦ剰鍔涙ā鍧?    
#     鍒嗙閲戝睘琛ㄩ潰鐨勬甯哥汗鐞嗗拰缂洪櫡鐗瑰緛
#     """
    
#     def __init__(self, channels):
#         super().__init__()
        
#         # 绾圭悊璺緞锛堜綆棰戯紝澶ф牳锛?#         self.texture_path = nn.Sequential(
#             nn.AvgPool2d(5, 1, 2),
...
#         # 缂洪櫡璺緞锛堥珮棰戯紝灏忔牳锛?#         self.defect_path = nn.Sequential(
#             nn.Conv2d(channels, channels // 2, 3, 1, 1, groups=channels // 2),
#             nn.BatchNorm2d(channels // 2),
#             nn.ReLU(inplace=True),
#             nn.Conv2d(channels // 2, channels, 1),
#             nn.Sigmoid()
...
#     def forward(self, x):
#         # 鍒嗙绾圭悊鍜岀己闄风壒寰?#         texture_weight = self.texture_path(x)
#         defect_weight = self.defect_path(x)
        
#         # 鍔犳潈鐗瑰緛
#         texture_feat = x * texture_weight
#         defect_feat = x * defect_weight
        
#         # 铻嶅悎
#         out = self.fusion(torch.cat([texture_feat, defect_feat], 1))
        
#         return out

class ECAAttention(nn.Module):
    """
    Efficient Channel Attention (ECA-Net CVPR2020)
    浼樺娍锛?    - 鏃犻檷缁达紝鐩存帴1D鍗风Н
    - 鍙傛暟閲忔瀬灏忥紙<1K锛?    - 鏁堟灉鎺ヨ繎SE-Net
    鍙傛暟锛?        channels: 杈撳叆閫氶亾鏁?        gamma, b: 鑷€傚簲鏍稿ぇ灏忕殑瓒呭弬鏁?    """
    def __init__(self, channels, gamma=2, b=1):
        super().__init__()
        # 鑷€傚簲璁＄畻1D鍗风Н鏍稿ぇ灏?        t = int(abs((math.log2(channels) + b) / gamma))
        k_size = t if t % 2 else t + 1
        k_size = max(3, k_size)  # 鏈€灏忎负3
...
class C2f_iRMB(C2f):
    def __init__(
        ...
        if use_channel_attn and n >= 2:
            concat_channels = (2 + n) * self.c
            self.channel_attn = ECAAttention(concat_channels)
        else:
    ...
    def forward(self, x):
        ...
        # Optional ECA channel attention.
        if self.channel_attn is not None:
            concat_feat = self.channel_attn(concat_feat)
...
class ECALite(nn.Module):
    def __init__(self, c: int, k_size: Optional[int] = None, gamma: int = 2, b: int = 1):
        super().__init__()
        if k_size is None:
...
# ---- (B) 杞婚噺澶ф牳绌洪棿娉ㄦ剰鍔涳細LKA-lite锛圖W 5x5 + dilated DW 7x7 + PW锛?---
class LKALite(nn.Module):
    def __init__(self, c: int, k: int = 5, dilated_k: int = 7, dilation: int = 3):
        super().__init__()
        self.dw1 = nn.Conv2d(c, c, k, 1, k // 2, groups=c)
        pad2 = (dilated_k // 2) * dilation
        self.dw2 = nn.Conv2d(c, c, dilated_k, 1, pad2, groups=c, dilation=dilation)
        self.pw  = nn.Conv2d(c, c, 1, 1, 0)
        self.act = nn.Sigmoid()
...
# ---- (C) ILA锛氭妸涓や釜杞绘敞鎰忓姏鍋氣€滄畫宸棬鎺р€濊瀺鍚堬紝鍒濆€尖増鎭掔瓑 ----
class IRMA_LiteAttention(nn.Module):
    def __init__(self, c: int, eca_k: Optional[int] = None,
                 lka_k: int = 5, lka_dk: int = 7, lka_d: int = 3,
                 drop_p: float = 0.0):
        super().__init__()
        self.ca = ECALite(c, k_size=eca_k)
        self.sa = LKALite(c, k=lka_k, dilated_k=lka_dk, dilation=lka_d)
    ...
    def forward(self, x):
        ca = self.ca(x) - x
        sa = self.sa(x) - x
        a  = torch.sigmoid(self.alpha) * ca + torch.sigmoid(self.beta) * sa
        return x + self.drop(a)
    
class AttnFuse(nn.Module):
    """鐢?涓潪璐熸潈閲嶅仛鍔犳潈鍜岋紝閬垮厤鐩茬洰Concat"""
    # 鏇挎崲concat锛屾湁娉ㄦ剰鍔涘紩瀵艰繘琛岃瀺鍚?
    def __init__(self, c: int, eps: float = 1e-4):
        super().__init__()
        self.w = nn.Parameter(torch.ones(2))
        self.eps = eps
        self.post = nn.Conv2d(c, c, 1, 1, 0)  # 缁村害鏍℃/杞绘贩鍚?
    def forward(self, x_td, x_lat):
        w = torch.relu(self.w)
        w = w / (w.sum() + self.eps)
        y = w[0] * x_td + w[1] * x_lat
        return self.post(y)

class AttnFuseCat(nn.Module):
    """
    鎺ユ敹 *宸茬粡 Concat 涔嬪悗* 鐨勫崟璺壒寰侊紝鍋氳交鏉冮噸鎶曞奖鍒板浐瀹?c2銆?    - 鎳掓瀯寤猴細绗竴娆?forward锛堟垨杈撳叆閫氶亾鍙樺寲锛夋椂锛岀敤 x.shape[1] 鍒涘缓 1x1 conv锛?    - 淇濊瘉杈撳嚭閫氶亾 == c2锛堜笌 YAML 鐨?[512] 瀵归綈锛夛紱
    - 閬垮厤 parse_model 璁や负鏄?512锛岃€屾ā鍧楀疄闄呯敤鐨勬槸鍏朵粬閫氶亾锛?92/384 绛夛級銆?    """
    def __init__(self, c2: int, use_bn: bool = True, act: bool = True):
        super().__init__()
        self.c2 = int(c2)
        self.use_bn = use_bn
        self.act = nn.SiLU(inplace=True) if act else nn.Identity()

        self.in_channels = None   # 褰撳墠鏉冮噸瀵瑰簲鐨?in_ch
        self.proj = None          # nn.Conv2d(in_ch, c2, 1, 1)
        self.bn = None

    def _rebuild(self, c1: int, device, dtype):
        self.in_channels = int(c1)
        self.proj = nn.Conv2d(self.in_channels, self.c2, kernel_size=1, stride=1, padding=0, bias=False).to(device=device, dtype=dtype)
        self.bn = nn.BatchNorm2d(self.c2).to(device) if self.use_bn else None

    def forward(self, x: torch.Tensor):
        c1 = int(x.shape[1])
        if (self.proj is None) or (self.in_channels != c1):
            # 绗竴娆?or 杈撳叆閫氶亾鍙樹簡 => 閲嶆柊鏋勫缓涓€娆?
            self._rebuild(c1, x.device, x.dtype)
...
# =========================
# v2锛氬湪姣忎釜 iRMB 瀛愬潡鍚庢寕杞婚噺娉ㄦ剰鍔涳紙榛樿 ILA锛?# =========================
def _build_lite_attn(c: int, attn: Optional[str], cfg: Optional[Dict[str, Any]]):
    attn = (attn or "off").lower()
    cfg = cfg or {}
    if attn in ("off", "none"):
        return nn.Identity()
    if attn == "ila":
        return IRMA_LiteAttention(
            c,
            eca_k=cfg.get("eca_k", None),
            lka_k=cfg.get("lka_k", 5),
            lka_dk=cfg.get("lka_dk", 7),
            lka_d=cfg.get("lka_d", 3),
            drop_p=cfg.get("drop_p", 0.0),
        )
    if attn == "lka":
        return LKALite(c, k=cfg.get("lka_k", 5), dilated_k=cfg.get("lka_dk", 7), dilation=cfg.get("lka_d", 3))
    if attn == "eca":
        return ECALite(c, k_size=cfg.get("eca_k", None))
    return nn.Identity()

class C2f_iRMB_DRBv2(C2f):
    """
    C2f 澶栧３ + iRMB_DRB 鍫嗗彔锛涘彲閫夊悗缃交娉ㄦ剰鍔涖€傛敞鎰忥細C2f 鐨勮緭鍑洪€氶亾鏄?c2锛?    """
    def __init__(self, c1, c2, n=1, kernel_size=None, shortcut=False, g=1, e=0.5,
                 attn_mode="none", attn_cfg=None, drop_path=0.0):
        super().__init__(c1, c2, n, shortcut, g, e)

        # 1) iRMB 鍫嗗彔 鈥?鎸?C2f 鐨勯殣钘忛€氶亾 self.c 鏋勫缓鍐呴儴妯″潡锛堜繚鎸佷笌瀹樻柟 C2f 鐢ㄦ硶涓€鑷达級
        self.m = nn.ModuleList(iRMB_DRB(self.c, self.c, dw_ks=kernel_size) for _ in range(n))

        # 2) 杞绘敞鎰忓姏锛堟斁鍦ㄨ緭鍑轰笂锛岄€氶亾鏁板繀椤荤瓑浜庤緭鍑洪€氶亾 c2锛?        attn_mode = (attn_mode or "none").lower()
        attn_cfg = attn_cfg or {}
        attn_channels = c2  # 鈫?鍏抽敭锛氭敞鎰忓姏鎸夌収鏈€缁堣緭鍑洪€氶亾鏉ュ缓
        if attn_mode == "ila":
            self.attn = IRMA_LiteAttention(attn_channels, **attn_cfg)
        elif attn_mode == "eca":
            self.attn = ECALite(attn_channels, **attn_cfg)
        elif attn_mode == "lka":
            self.attn = LKALite(attn_channels, **attn_cfg)
        else:
    ...
    def forward(self, x):
        ...
        y = self.attn(y)                  # 娉ㄦ剰鍔涗笌杈撳嚭閫氶亾涓€鑷?        return y


class C2f_iRMB_SWCv2(C2f):
    """
    C2f 澶栧３ + iRMB_SWC 鍫嗗彔锛涘彲閫夊悗缃交娉ㄦ剰鍔涖€傛敞鎰忓姏閫氶亾鎸?c2 鍒濆鍖栥€?    """
    def __init__(self, c1, c2, n=1, kernel_size=None, shortcut=False, g=1, e=0.5,
                 attn_mode="none", attn_cfg=None, drop_path=0.0):
        super().__init__(c1, c2, n, shortcut, g, e)
        ...
        attn_mode = (attn_mode or "none").lower()
        attn_cfg = attn_cfg or {}
        attn_channels = c2  # 鈫?鍏抽敭
        if attn_mode == "ila":
            self.attn = IRMA_LiteAttention(attn_channels, **attn_cfg)
        elif attn_mode == "eca":
            self.attn = ECALite(attn_channels, **attn_cfg)
...
# ------------------------------------------------------------
# Top-1 CondConv: 鍙墽琛屼竴涓笓瀹讹紱鍚屼笓瀹舵牱鏈垎缁勪竴娆″嵎绉?# ------------------------------------------------------------
class Top1CondConv2d(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=1, stride=1, padding=None,
                 dilation=1, groups=1, bias=False, num_experts=4,
                 use_gumbel=True, tau=1.0, deploy=False):
        super().__init__()
        self.in_ch, self.out_ch = in_ch, out_ch
        self.ks, self.s, self.d = kernel_size, stride, dilation
        self.p = autopad(kernel_size, padding, dilation)
        self.g = groups
        self.E = num_experts
        self.use_gumbel, self.tau, self.deploy = use_gumbel, tau, deploy

        self.router = nn.Linear(in_ch, num_experts, bias=True)
        w_shape = (self.E, out_ch, in_ch // groups, kernel_size, kernel_size)
        self.weight = nn.Parameter(torch.Tensor(*w_shape))
        self.bias = nn.Parameter(torch.zeros(self.E, out_ch)) if bias else None
        nn.init.kaiming_normal_(self.weight, mode="fan_out", nonlinearity="relu")

    def _route(self, x):
        pooled = F.adaptive_avg_pool2d(x, 1).flatten(1)  # [B, Cin]
        logits = self.router(pooled)                     # [B, E]
        if self.training and not self.deploy and self.use_gumbel:
            onehot = gumbel_softmax(logits, tau=self.tau, hard=True, dim=1)
            sel = onehot.argmax(1)  # [B]
        else:
            sel = logits.argmax(1)
        buckets = []
        for e in range(self.E):
            idx = torch.nonzero(sel == e, as_tuple=False).squeeze(1)
            buckets.append(idx)
        return sel, buckets

    def forward(self, x):
        B, _, H, W = x.shape
        kh = kw = self.ks if isinstance(self.ks, int) else self.ks[0]
        ph = pw = self.p if isinstance(self.p, int) else self.p[0]
        Oh, Ow = _out_size(H, kh, self.s, ph, self.d), _out_size(W, kw, self.s, pw, self.d)

        out = x.new_zeros((B, self.out_ch, Oh, Ow))
        _, buckets = self._route(x)

        for e, idx in enumerate(buckets):
            if idx.numel() == 0:
                continue
            xb = x.index_select(0, idx)
            wb = self.weight[e]
            bb = None if self.bias is None else self.bias[e]
            yb = F.conv2d(xb, wb, bb, self.s, (ph, pw), self.d, self.g)
            out.index_copy_(0, idx, yb)
        return out

    @torch.no_grad()
    def switch_to_deploy(self, tau=0.5):
        self.deploy = True
        self.tau = tau

# ------------------------------------------------------------
# 浣犵粰鐨?DynamicConv_Single 鈫?Top-1 闂ㄦ帶鐗堬紙绛惧悕鍏煎锛?# ------------------------------------------------------------
class DynamicConv_Single_Top1(nn.Module):
    """ Top-1 Dynamic Conv layer """
    def __init__(self, in_features, out_features, kernel_size=1, stride=1, padding='',
                 dilation=1, groups=1, bias=False, num_experts=4,
                 use_gumbel=True, tau=1.0, deploy=False):
        super().__init__()
        self.cond_conv = Top1CondConv2d(in_features, out_features, kernel_size, stride, padding,
                                        dilation, groups, bias, num_experts,
                                        use_gumbel=use_gumbel, tau=tau, deploy=deploy)

    def forward(self, x):
        return self.cond_conv(x)

class DynamicConv_Top1(nn.Module):
    default_act = nn.SiLU()

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True,
                 num_experts=4, use_gumbel=True, tau=1.0, deploy=False, bias=False):
        super().__init__()
        # 椤哄簭锛氶棬鎺у嵎绉?鈫?BN 鈫?婵€娲?
        self.conv = nn.Sequential(
            DynamicConv_Single_Top1(
                c1, c2,
                kernel_size=k, stride=s, padding=autopad(k, p, d),
                dilation=d, groups=g, bias=bias, num_experts=num_experts,
                use_gumbel=use_gumbel, tau=tau, deploy=deploy
            ),
            nn.BatchNorm2d(c2),
            self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        )
        # 鍏抽敭锛氫繚瀛樺彾瀛愰棬鎺у眰鐨勫紩鐢紝鍚庣画鍙垏瀹?
        self.cond = self.conv[0]

    def forward(self, x):
        return self.conv(x)

    def switch_to_deploy(self, tau=0.5):
        """鎶婇棬鎺у垏鍒扮‖璺敱锛堝彧瀵瑰彾瀛愰棬鎺у眰鐢熸晥锛岄伩鍏嶉€掑綊锛?"""
        if getattr(self, "_switched", False):
            return
        self._switched = True
        if hasattr(self.cond, "switch_to_deploy"):
            self.cond.switch_to_deploy(tau)
        # 涓嶉渶瑕?return 璇彞

# ------------------------------------------------------------
# Ghost 妯″潡锛堜袱鏉℃敮璺兘鐢?Top-1 鍔ㄦ€佸嵎绉級
# ------------------------------------------------------------
class GhostModule_Top1(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1,
                 act_layer=nn.SiLU, num_experts=2, use_gumbel=True, tau=1.0, deploy=False):
        super().__init__()
        self.oup = oup
        init_channels = math.ceil(oup / ratio)
        new_channels  = init_channels * (ratio - 1)

        self.primary_conv = DynamicConv_Top1(inp, init_channels, kernel_size, stride,
                                             num_experts=num_experts, use_gumbel=use_gumbel,
                                             tau=tau, deploy=deploy)
        # depthwise 浣滀负寤変环鎿嶄綔锛坓roups=init_channels锛?
        self.cheap_operation = DynamicConv_Top1(init_channels, new_channels, dw_size, 1,
                                                g=init_channels, num_experts=num_experts,
                                                use_gumbel=use_gumbel, tau=tau, deploy=deploy)
        self.act = act_layer()
    ...
    def switch_to_deploy(self, tau=0.5):
        self.primary_conv.switch_to_deploy(tau)
        self.cheap_operation.switch_to_deploy(tau)

# ------------------------------------------------------------
# 涓?YOLOv8 鐨?C2f/C3/Bottleneck 瀵规帴锛堝悗缂€ _Top1锛夋寜鐓т竴涓笓瀹剁綉缁滆繘琛岃瘑鍒?# ------------------------------------------------------------
from ultralytics.nn.modules import C2f, C3, Bottleneck  # 淇濇寔涓庨」鐩竴鑷?
class Bottleneck_DynamicConv_Top1(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5,
                 num_experts=2, use_gumbel=True, tau=1.0, deploy=False):
        super().__init__(c1, c2, shortcut, g, k, e)
        self.cv2 = DynamicConv_Top1(c2, c2, 3, num_experts=num_experts,
                                    use_gumbel=use_gumbel, tau=tau, deploy=deploy)

class C3_DynamicConv_Top1(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5,
                 num_experts=2, use_gumbel=True, tau=1.0, deploy=False):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(Bottleneck_DynamicConv_Top1(
            c_, c_, shortcut, g, k=(1, 3), e=1.0,
            num_experts=num_experts, use_gumbel=use_gumbel, tau=tau, deploy=deploy
        ) for _ in range(n)))

class C2f_DynamicConv_Top1(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5,
                 num_experts=2, use_gumbel=True, tau=1.0, deploy=False):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_DynamicConv_Top1(
            self.c, self.c, shortcut, g, k=(3, 3), e=1.0,
            num_experts=num_experts, use_gumbel=use_gumbel, tau=tau, deploy=deploy
        ) for _ in range(n))

class C3_GhostDynamicConv_Top1(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5,
                 ratio=2, num_experts=2, use_gumbel=True, tau=1.0, deploy=False):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(GhostModule_Top1(
            c_, c_, ratio=ratio, num_experts=num_experts,
            use_gumbel=use_gumbel, tau=tau, deploy=deploy
        ) for _ in range(n)))

class C2f_GhostDynamicConv_Top1(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5,
                 ratio=2, num_experts=2, use_gumbel=True, tau=1.0, deploy=False):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(GhostModule_Top1(
            self.c, self.c, ratio=ratio, num_experts=num_experts,
            use_gumbel=use_gumbel, tau=tau, deploy=deploy
        ) for _ in range(n))

##浠ヤ笅鍔犱笂1X1鐨勫榻愬眰锛屼笉闇€瑕侀鍏堢煡閬撹緭鍏ラ€氶亾锛屽皝瑁呭湪涓€涓粍鍚堝崟鍏冧箣鍚庡啀鐢╕AML涔嬪悗
class Conv1x1Align(nn.Module):
    def __init__(self, c_out, act=True):
        super().__init__()
        self.c_out = c_out
        self.conv = None; self.bn = None
        self.act = nn.SiLU() if act else nn.Identity()
    def forward(self, x):
        if self.conv is None:
            self.conv = nn.Conv2d(x.shape[1], self.c_out, 1, 1, 0, bias=False).to(x.device)
            self.bn   = nn.BatchNorm2d(self.c_out).to(x.device)
        return self.act(self.bn(self.conv(x)))

class C2f_GhostDynamicConv_Top1_Align(nn.Module):
    # 瀵归綈(1x1) + 浣犲師鏉ョ殑 C2f_GhostDynamicConv_Top1
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.align = Conv1x1Align(c2)           # 鍏堟妸 concat 鍚庣殑閫氶亾瀵归綈鎴?c2
        self.body  = C2f_GhostDynamicConv_Top1(c2, c2, n=n, shortcut=shortcut, g=g, e=e)
    def forward(self, x):
        x = self.align(x)
        return self.body(x)
...
# 鑷繁淇敼鐨凟UCB妯″潡
class EUCB_Lite(nn.Module):
    def __init__(self, in_channels, out_channels=None, kernel_size=3, stride=1, shuffle_groups=2):
        super(EUCB_Lite, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels or in_channels
        self.shuffle_groups = shuffle_groups

        # 鏇夸唬 Upsample锛屼娇鐢ㄦ爣鍑嗚浆缃嵎绉紝鏀寔閲忓寲鍔犻€?
        self.up_conv = nn.Sequential(
            nn.ConvTranspose2d(in_channels, in_channels, kernel_size=2, stride=2, padding=0, groups=1, bias=False),
            nn.BatchNorm2d(in_channels),
            nn.ReLU(inplace=True)
        )

        # 閫氶亾閲嶇粍锛堥€氳繃reshape瀹炵幇锛岄伩鍏峆ython鎿嶄綔锛?
        self.channel_shuffle = nn.Identity() if shuffle_groups <= 1 else ChannelShuffle(shuffle_groups)
...
class ScharrConv(nn.Module):
    def __init__(self, channel):
        ...
        # 灏哠charr鏍歌浆鎹负PyTorch寮犻噺骞舵墿灞曚负閫氶亾鏁?
        ...
        # 瀹氫箟鍗风Н灞傦紝浣嗕笉瀛︿範鍗风Н鏍革紝鐩存帴浣跨敤Scharr鏍?
...
# Unified EUCB_SC implementation (export-friendly).
# Legacy experimental/commented EUCB_SC variants were removed for clarity and maintainability.

class EUCB_SC(nn.Module):
    """Rep-HFE: nearest upsample -> 3x3 depthwise conv -> 1x1 conv -> SiLU."""

    def __init__(self, c1, c2=None, scale: int = 2, groups: int = 4, shift_size: int = 1):
        super().__init__()
        self.c1 = int(c1)
        self.c2 = int(c2 if c2 is not None else c1)
        self.scale = int(scale)
        self.up = nn.Upsample(scale_factor=self.scale, mode="nearest")
        self.dw = Conv(self.c1, self.c1, k=3, s=1, g=self.c1, act=False)
        self.pw = Conv(self.c1, self.c2, k=1, s=1, g=1, act=False)
